...
/**
 * Alinea tiempos de turnos (cerrados y en curso) en planilla con montaje_time_segments
...
class MontajeTurnosSegmentSyncService
{
    ...
    private const ACTUAL_KEY = 'montTurnoActual';

    private const SYNC_NOTE_PREFIX = 'mont_turno_sync:';

    /**
     * Turnos Montaje abiertos en planilla (cronómetro en curso), por OT.
     *
    ...
    private function syncOneTurno(
        ...
        $mountSec = $durations['arranque'] + $durations['montaje_op'];
        if ($mountSec >= 0.01) {
            $segStart = $cursor->copy()->subSeconds((int) round($mountSec));
            $this->createSegment(
                $workOrder,
                $user,
                $machineCode,
    ...
    /**
     * @param  array{effective: float, dead: float, arranque: float, montaje_op: float, demount: float, total: float}  $durations
    ...
    private function durationsToLiveRows(array $durations, array $timer, string $machineCode): array
    {
        ...
        $append('mount', $durations['arranque'] + $durations['montaje_op']);
    ...
    /**
     * @param  array<string, mixed>  $timer
     * @return array{effective: float, dead: float, arranque: float, montaje_op: float, demount: float, total: float}
     */
    private function resolveTimerDurations(array $timer, ?Carbon $now): array
    {
        ...
        $montajeOp = max(0.0, (float) ($timer['montajeOpAccSec'] ?? 0));
        ...
        $montajeOp += $this->runningPhaseSeconds(
            (string) ($timer['montajeOpState'] ?? 'idle'),
            (int) ($timer['montajeOpLastResumeAtMs'] ?? 0),
            $nowMs,
        );
        ...
        $total = $effective + $dead + $arranque + $montajeOp + $demount;

        return [
            'effective' => $effective,
            'dead' => $dead,
            'arranque' => $arranque,
            'montaje_op' => $montajeOp,
            'demount' => $demount,
            'total' => $total,
        ];
    }
