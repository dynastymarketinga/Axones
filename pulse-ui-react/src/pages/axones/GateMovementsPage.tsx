...
export default function GateMovementsPage() {
  ...
  const [rows, setRows] = useState<LaravelPaginated<GateRow> | null>(null)

  const load = useCallback(async () => {
    setLoading(true)
    try {
      const data = await apiFetch<LaravelPaginated<GateRow>>("gate-movements", {
        query: {
          page,
          per_page: 20,
      ...
      else toast.error("No se pudieron cargar los movimientos de vigilancia.")
      setRows(null)
    } finally {
      setLoading(false)
    }
            ...
            {loading ? (
              ...
              rows.data.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell>
                    {r.occurred_at
                      ? String(r.occurred_at).slice(0, 19).replace("T", " ")
                      : "—"}
                  </TableCell>
