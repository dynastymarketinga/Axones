...
import { ProductionTimeOtMobileCards } from "./production-time-ot-mobile-cards"
    ...
    <div className="bg-card space-y-3 rounded-2xl border p-3 shadow-sm sm:p-4">
      <div className="space-y-1">
        ...
        <p className="text-muted-foreground text-xs leading-relaxed">
          Pulse una fila o tarjeta para seleccionar la OT. Use <strong>Abrir</strong> para la vista previa con
          detalle de paradas, o <strong>Ver OT</strong> para abrir la planilla. Los tiempos suman montaje, impresión,
          laminación y corte en el período
          {includeLive
            ? " e incluyen turnos en curso (actualización cada 30 s)."
            : " (solo segmentos cerrados)."}
        </p>
      </div>

      <ProductionTimeOtMobileCards
        candidates={candidates}
        loading={loading}
        selectedWoId={selectedWoId}
        aggregateAll={aggregateAll}
        emptyMessage={REPORT_EMPTY_WORK_ORDER_TIMES}
      ...
      <div className="hidden lg:block">
        <ReportTableScroll tableMinWidthClass="min-w-[70rem]">
          <Table>
            <TableHeader>
                    ...
                    >
                      ...
                      <TableCell className={catalogTableBodyCellClass}>
                        <div className="flex flex-wrap gap-1">
                          {row.areas.filter(isProductionTimeReportArea).map((a) => (
                      ...
                      <TableCell className={cn("text-right", catalogTableBodyCellClass)}>
                        <div className="flex flex-wrap justify-end gap-1">
                          ...
                          <Button asChild variant="outline" size="sm" className="border-primary/25">
                            <Link
                              to={`/ordenes-trabajo/${row.work_order_id}`}
