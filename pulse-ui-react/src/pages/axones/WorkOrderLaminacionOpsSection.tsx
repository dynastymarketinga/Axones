...
export default function WorkOrderLaminacionOpsSection(props: Props) {
  ...
  const doneProduccion =
    num(props.totalSalidaTurno) > 0 || num(props.scrapLaminadoRaw) > 0 || num(props.metrajeRaw) > 0
    ...
    >
      ...
      <div className="mes-footer-bar mes-footer-bar--3">
        ...
        <div className="mes-footer-bar__item flex items-start gap-2">
          ...
          <span>
            Merma acumulada (Kg): <strong>{props.totalScrapAcumulada.toFixed(2)} Kg</strong>
          </span>
        </div>
        <div className="mes-footer-bar__item flex items-start gap-2">
  ...
  return (
    <>
              ...
              {props.closedTurnos.map((t) => (
                <li key={t.id} className="rounded border bg-background p-2">
                  ...
                  <div className="text-muted-foreground mt-1">
                    Producción {sumSalidaKgTurno(t).toFixed(2)} Kg · Salida {sumSalidaKgTurno(t).toFixed(2)} Kg · Tiempo efectivo{" "}
                ...
                {!simplifiedTimer && props.canFinalizeOrder && !props.areaFinalizada ? (
                  <div className="mes-timer-action-labeled">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent side="right" className="max-w-xs">
                        Cierra el área de laminación en la orden (paso de gestión). No sustituye a «Cerrar turno» ni a
      ...
      <>
      <MesSectionShell
        title={mesSectionTitle(Weight, "Salida laminada del turno")}
        subtle
        headerRight={<MesSectionHeaderExtras isDone={doneProduccion} />}
      >
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="space-y-1">
            {fieldLabel(mk("kg-prod"), Weight, "Metraje producción")}
            <Input
              id={mk("kg-prod")}
              name="lamSalidaKg"
              className="ot-input-unified h-9"
              inputMode="decimal"
              value={props.totalSalidaTurno}
