...
class LaminacionProductionService
{
     ...
     */
    public function getLaminacionState(WorkOrder $workOrder): array
    {
        $workOrder->loadMissing(['client', 'product', 'clientOrder']);

        $summary = WorkOrderLaminacionSummary::query()->where('work_order_id', $workOrder->getKey())->first();

        $openSegment = LaminacionTimeSegment::query()
            ->where('work_order_id', $workOrder->getKey())
            ->whereNull('ended_at')
            ->with('user:id,name,email')
            ->first();

        $recentSegments = LaminacionTimeSegment::query()
            ->where('work_order_id', $workOrder->getKey())
            ->with('user:id,name,email')
        ...
        $bobinaUsages = LaminacionBobinaUsage::query()
            ->where('work_order_id', $workOrder->getKey())
        ...
        $closed = LaminacionTimeSegment::query()
            ->where('work_order_id', $workOrder->getKey())
    ...
    public function startTimeSegment(WorkOrder $workOrder, User $user, string $segmentType, ?string $notes = null, ?string $machineCode = null): LaminacionTimeSegment
    {
        if ($workOrder->status === WorkOrderStatus::Cancelled->value) {
            throw ValidationException::withMessages([
                'work_order_id' => ['No se puede registrar laminación en una orden cancelada.'],
            ]);
        }
        ...
        return DB::transaction(function () use ($workOrder, $user, $segmentType, $notes, $machineCode) {
            $open = LaminacionTimeSegment::query()
                ->where('work_order_id', $workOrder->getKey())
                ->whereNull('ended_at')
                ->lockForUpdate()
                ->first();
            ...
            if ($open) {
                ...
                $this->alerts->evaluateClosedLaminacionTimeSegment($open->fresh());
            }

            return LaminacionTimeSegment::query()->create([
    ...
    public function stopTimeSegment(LaminacionTimeSegment $segment): LaminacionTimeSegment
    {
        ...
        $this->alerts->evaluateClosedLaminacionTimeSegment($segment->fresh());
     ...
     */
    public function storeBobinaUsage(WorkOrder $workOrder, array $data): LaminacionBobinaUsage
    {
        if ($workOrder->status === WorkOrderStatus::Cancelled->value) {
            throw ValidationException::withMessages([
        ...
        return LaminacionBobinaUsage::query()->create([
            'work_order_id' => $workOrder->getKey(),
            'bobina_id' => $bobinaId,
    ...
    /**
     * @param  array{scrap_percent?: float|string|null, solvent_quantity_kg?: float|string|null, solvent_notes?: string|null, notes?: string|null}  $data
     */
    public function upsertSummary(WorkOrder $workOrder, array $data): WorkOrderLaminacionSummary
    {
        if ($workOrder->status === WorkOrderStatus::Cancelled->value) {
            throw ValidationException::withMessages([
        ...
        $summary = WorkOrderLaminacionSummary::query()->firstOrNew(['work_order_id' => $workOrder->getKey()]);
        $scrapUpdated = array_key_exists('scrap_percent', $data);
        if ($scrapUpdated) {
            $summary->scrap_percent = $data['scrap_percent'];
        }
        if (array_key_exists('solvent_quantity_kg', $data)) {
            $summary->solvent_quantity_kg = $data['solvent_quantity_kg'];
        }
        if (array_key_exists('solvent_notes', $data)) {
            $summary->solvent_notes = $data['solvent_notes'];
        }
        ...
        if ($scrapUpdated) {
            $this->alerts->evaluateScrapPercent($workOrder, $summary->scrap_percent, 'laminación');
        }
