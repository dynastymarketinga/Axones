import type { PrintingPlanillaSheet } from "@/lib/printing-planilla-preview"
...
function turnoMark(sheet: PrintingPlanillaSheet, key: "D" | "1" | "2" | "3"): boolean {
...
function SectionSubmeta({ sheet }: { sheet: PrintingPlanillaSheet }) {
  return (
...
type Props = {
  sheet: PrintingPlanillaSheet
  sheetIndex: number
  sheetTotal: number
}

export function PrintingPlanillaPaperSheet({ sheet, sheetIndex, sheetTotal }: Props) {
  const entradaCells = Array.from({ length: ENTRADA_SLOTS }, (_, i) => sheet.entrada_laminas[i] ?? "")
  const salidaCells = Array.from({ length: SALIDA_SLOTS }, (_, i) => sheet.salida_bobinas[i] ?? "")

  return (
    <article className="printing-planilla-paper-page" aria-label={`Planilla impresión turno ${sheetIndex + 1}`}>
      {sheetTotal > 1 ? (
      ...
      <header className="pp-doc-header">
        ...
        <div className="pp-doc-center">
          ...
          <h1 className="pp-doc-title">Control de producción de impresión</h1>
        </div>
      </header>
          ...
          <table className="pp-head-table">
            <tbody>
              ...
              <tr>
                ...
                <td colSpan={2} className="pp-field-line">
                  <span className="pp-inline-fixed">Impresora</span>
              ...
              <tr>
                ...
                <td colSpan={3} className="pp-field-line">
                  {sheet.hora_arranque || ""}
                </td>
              </tr>
            </tbody>
      ...
      <div className="pp-section-title">Ingreso de material (laminas)</div>
          ...
          <table className="pp-pesaje-table">
            ...
            <tbody>
              {Array.from({ length: SALIDA_LEFT }, (_, row) => {
                ...
                return (
                  <tr key={`sal-${row}`}>
                    <td className="pp-proc">IMP. X LAM.</td>
